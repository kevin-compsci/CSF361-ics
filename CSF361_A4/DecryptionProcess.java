...
public class DecryptionProcess {
	...
	/* startDecryption void function will initiate runs to encrypt the file */
	public void startDecryption(int[][] plaintext, int[][] key, String fileName) throws IOException {
		int rounds = 0;
	...
	public int subBytes(int item) {
		...
		return data.inverseSBOX[higherNibble][lowerNibble];
	}
	...
	public int[][] executeMixColumns(int[][] plaintext) {
		...
		while (columns < 4) {
			plaintext = mixCol.invMixColumn2(columns, plaintext);
			columns++;
		}
		return plaintext;
	}
		...
		while (rows < 4) {
			while (columns < 4) {
				if (plaintext[rows][columns] >= 1 && plaintext[rows][columns] <= 15) {
					decryptedMessage += "0" + Integer.toHexString(plaintext[rows][columns]) + "";
				}
				else {
					decryptedMessage += Integer.toHexString(plaintext[rows][columns]) + "";
				}
				columns++;
			}
			columns = 0;
